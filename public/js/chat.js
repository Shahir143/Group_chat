        ...
        document.querySelectorAll("[data-conversation]").forEach(function (item) {
            item.addEventListener("click", async function (e) {
                ...
                if(!chatSockets[chatId]){
                    chatSockets[chatId]=io(`${baseUrl}`,{
                        auth:{
                            token:token,
                        }
                    })
                    chatSockets[chatId].on('receive-message',(msg)=>{
                        console.log("msgin chatsockets",msg);
                        createmessage(msg,profile_picture)
                    })
                    chatSockets[chatId].on('group-message',(msg)=>{
                        console.log("msgin grpsockets",msg);
                        createmessage(msg,profile_picture)
                    })
                }
...
async function generateHead(userData){
    try {
        ...
        const formattedDate = `${crtTime(lastSeenDate.getHours())}:${crtTime(lastSeenDate.getMinutes())}:${crtTime(lastSeenDate.getSeconds())}`;
...
async function createmessage(item,data){
    ...
    try{
        if(item.isAttachment===true){
            ...
            imageUrl.classList="sendMessageImage"
            ...
            if (item.messageStatus === "received" && groupid) {
                console.log('group',groupid);
                image.src = `https://cdn6.aptoide.com/imgs/1/2/2/1221bc0bdd2354b42b293317ff2adbcf_icon.png`;
            }
			if (item.messageStatus === "received" && reciverid ) {
                console.log('chat',reciverid)
				listItem.classList.add("conversation-item", "me");
...
async function sendChat(id){
    ...
    chatSockets[id].emit("send-message", messageDetails);
        ...
        document.querySelectorAll("[data-conversation]").forEach(function (item) {
            item.addEventListener("click", async function (e) {
                ...
                if(!groupSockets[groupId]){
                    groupSockets[groupId]=io(`${baseUrl}`,{
                        auth:{
                            token:token,
                        }
                    });
                    groupSockets[groupId].on('group-message',(newGrpMessage)=>{
                        createmessage(newGrpMessage,profile_picture);
                    })
                }
                // Remove 'active' class from all conversations
                document.querySelectorAll(".conversation").forEach(function (i) {
...
async function getGroupApi(groupId){
    try{
        const response=await axios.get(`${baseUrl}/chat/getGroupchat/${groupId}`,{
            headers:{Authorization:token},
        })
        if(response.status===200){
...
async function getSelfDetails() {
	const selfDetails = await axios.get(`${baseUrl}/user/self`, { headers: { Authorization: token } });
	profile_picture = selfDetails.data;
